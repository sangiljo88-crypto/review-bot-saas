...
const ENCRYPT_KEY = process.env.SESSION_ENCRYPT_KEY || "";
const IV_LENGTH = 16;

function getEncryptKeyBuffer() {
  if (Buffer.byteLength(ENCRYPT_KEY, "utf8") !== 32) {
    throw new Error("SESSION_ENCRYPT_KEY는 정확히 32바이트 문자열이어야 합니다.");
  }
  return Buffer.from(ENCRYPT_KEY, "utf8");
}

function encrypt(text) {
  const iv = crypto.randomBytes(IV_LENGTH);
  const cipher = crypto.createCipheriv("aes-256-cbc", getEncryptKeyBuffer(), iv);
...
function decrypt(text) {
  ...
  const decipher = crypto.createDecipheriv("aes-256-cbc", getEncryptKeyBuffer(), iv);
